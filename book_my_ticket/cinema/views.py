...
from .models import (
    City, Brand, Cinema,
    Screen, Movies, SeatCategory, Showtime, ScreenCapacity, Reservation, Booking,
)
from .serializers import (
    CitySerializer, BrandSerializer, CinemaSerializer,
    ScreenSerializer, MovieSerializer, SeatCategorySerializer,
    MoviesByCitySerialazer, ShowtimeSerializer, CinemaByMovieSerializer,
    ScreenCapacitySerializer, CinemaWithScreensOnly,
    ReservationSerializer, SeatBookingSerializer
)
...
class MoviesByCity(generics.GenericAPIView):
    ...
    def post(self, request):
        ...
        movies_id = Showtime.objects.filter(
            ...
            start_timestamp__range=(start, end)).values_list('movie', flat=True)
...
class CinemaByMovie(generics.GenericAPIView):
    ...
    def post(self, request):
        ...
        screens = Screen.objects.filter(id__in=showtimes.values_list('screen', flat=True))
        ...
        for cinema in _cinemas:
            ...
            for co in dict(cinema)["screens"]:
                _shows = list(filter(None, list(filter(lambda d: d['id'] in s_ids, dict(co)["showtimes"]))))
                if len(_shows) > 0:
                    shows.append(_shows)
...
def check_availability(showtime, seat_category=None):
    if not seat_category:
        seat_categories = (ScreenCapacity
                           .objects
                           .order_by('seat_category')
    ...
    screen_capacity = (ScreenCapacity
                       .objects
                       .filter(screen=showtime.screen, seat_category__in=seat_categories)
    ...
    for cap in screen_capacity:
        reserved = (Reservation
                    .objects
                    .filter(showtime=showtime.pk, seat_category=cap["seat_category"], showtime__status='CF')
                    .values('seat_category')
                    .annotate(booked_seat=Sum('seats'))
                    )
        if reserved.exists():
            available_seats = cap["no_of_seats"] - reserved.values('booked_seat')[0]['booked_seat']
        else:
            available_seats = cap["no_of_seats"]
        if not available_seats < 1:
            _resp = dict(seat_category=cap["seat_category"], available_seats=available_seats)
            resp.append(_resp)
    return resp
...
class SeatAvailability(generics.RetrieveAPIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        return Response({"Error": "Seats are not available"}, status=status.HTTP_412_PRECONDITION_FAILED)
...
class SeatBooking(generics.CreateAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if len(available_seats) == 0:
            return Response({"Message": "Seats are not available for given category"}, status=status.HTTP_406_NOT_ACCEPTABLE)

        if available_seats[0]["available_seats"] - payload["seats"] < -1:
            return Response({"Message": "Seats are not available"}, status=status.HTTP_406_NOT_ACCEPTABLE)
