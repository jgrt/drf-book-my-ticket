...
class ScreenCapacitySerializer(serializers.ModelSerializer):
    screen_id = serializers.PrimaryKeyRelatedField(
        queryset=Screen.objects.all(), source="screen.id"
    )
    ...
    def validate(self, data):
        ...
        if booked_show.exists():
            raise serializers.ValidationError(
                "Another Show already booked on given date and time"
            )

        if (
            Movies.objects.filter(pk=data["movie"].pk).values("runtime")[0]["runtime"]
            > total_time
        ):
            raise serializers.ValidationError(
                "Movie runtime is greater then selected showtime"
            )
...
class ScreenSerializer(serializers.ModelSerializer):
    cinema_id = serializers.PrimaryKeyRelatedField(
        queryset=Cinema.objects.all(), source="cinema.id"
    )
    screen_capacity = ScreenCapacitySerializer(read_only=True, many=True)
    showtimes = ShowtimeSerializer(
        many=True, read_only=True, source="available_showtimes"
    )
    ...
    def create(self, validated_data):
        screen = Screen.objects.create(
            name=validated_data["name"], cinema=validated_data["cinema"]["id"]
        )
        return screen
...
class CinemaSerializer(serializers.ModelSerializer):
    city_id = serializers.PrimaryKeyRelatedField(
        queryset=City.objects.all(), source="city.id"
    )
...
class ReservationSerializer(serializers.ModelSerializer):
    booking_id = serializers.PrimaryKeyRelatedField(
        queryset=Booking.objects.all(), source="booking.id"
    )
    showtime_id = serializers.PrimaryKeyRelatedField(
        queryset=Showtime.objects.all(), source="showtime.id"
    )
    seat_category_id = serializers.PrimaryKeyRelatedField(
        queryset=SeatCategory.objects.all(), source="seat_category.id"
    )
...
class SeatBookingSerializer(serializers.Serializer):
    ...
    seat_category = serializers.PrimaryKeyRelatedField(
        queryset=SeatCategory.objects.all(), source="seat_category.id"
    )
...
class MoviesByCitySerialazer(serializers.Serializer):
    city = serializers.PrimaryKeyRelatedField(
        queryset=Cinema.objects.all(), source="city.id"
    )
